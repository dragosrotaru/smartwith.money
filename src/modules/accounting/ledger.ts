...
export class JournalEntry {
  ...
  static create(description: Description, date: Date, lines: Line[], trxId?: UUID) {
    ...
    if (hasSameAccount.length > 0) return new Error('You cannot have multiple journal entry lines for the same account')
...
export class Ledger {
  ...
  public addEntry(entry: JournalEntry) {
    if (entry.accounts.some((account) => !this.accounts.hasAccountWithId(account)))
